...
"""
Koden undersøger den bedste regulariseringsstyrke ved at plotte RMSE for de forskellige delta-værdier.
Dertil er de valgte delta-værdier markeret på grafen.
"""
...
kspace_undersampled, mask_used = undersampling(fillkspace())
...
delta_values = np.logspace(-5, 2, 50)  # 50 values from 0.00001 to 100 on log scale
rmse_values = []
...
delta_markers = [0.001, 0.25, 1]
colors_list = ['royalblue', 'hotpink', 'lime']
labels = ['$\\delta=0.001$', '$\\delta=0.25$', '$\\delta=1$']
for d, c, l in zip(delta_markers, colors_list, labels):
    idx = np.argmin(np.abs(delta_values - d))
    ax.plot(delta_values[idx], rmse_values[idx], 'o', color=c, markersize=8, label=l)

ax.legend(title = "Chosen deltas", fontsize=11)
ax.grid(True)
plt.tight_layout()
plt.show()
