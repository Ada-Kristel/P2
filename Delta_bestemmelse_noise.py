...
"""
Koden finder den bedste regulariseringsstyrke for en bestem støjværdi
Den laver dertil en figur over RMSE mod delta-værdier, hvor den bedste delta-værdi er markeret.
"""
...
kspace = np.zeros((ky_size,kx),dtype=np.complex128)
...
kspace = fillkspace()
np.random.seed(37)
kspace = kspace + np.random.normal(0, 2E+05, kspace.shape) + 1j * np.random.normal(0, 2E+05, kspace.shape)

def samplingmask(ky_size, center_fraction = 0.30):
    """
    :param ky_size: antal rækker i datasættet
    :param center_fraction: bestemmer hvor stor en del af rækkerne, som centurm udgør
...
kspace_undersampled, mask_used = undersampling(kspace)
...
# indsamler delta værdier
delta_values = np.logspace(-5, 2, 1000)
rmse_values = []

for delta in delta_values:
    image_recon, _ = Leastsquares(kspace_undersampled, mask_used, delta=delta)
...
ax.set_xscale('log')
ax.set_xlabel('Delta (δ)', fontsize=13)
ax.set_ylabel('RMSE', fontsize=13)
ax.set_title('RMSE vs Regularization Strength (δ)', fontsize=14)

# finder og markerer den bedste delta-værdi
best_idx = np.argmin(rmse_values)
best_delta = delta_values[best_idx]
best_rmse = rmse_values[best_idx]
ax.plot(best_delta, best_rmse, 'o', color='red', markersize=8)
ax.annotate(
    f'Best delta\nδ={best_delta:.4f}\nRMSE={best_rmse:.4f}',
    xy=(best_delta, best_rmse),
    xytext=(best_delta / 4, best_rmse + 0.15),
    fontsize=12,
